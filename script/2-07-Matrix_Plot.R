...
for (i in 1:n_matrix) # Permet de parcourir les matrices une par une
{
  ...
  { 
    ...
    to_set <- which(gene_matrix[, 10] != 0) # On isole les lignes pour lesquelles le gene matche
    m <- gene_matrix[c(to_set), 10] # On extrait lesdites lignes
    
    png(str_glue("{deb_fr}{matrix_name[i]}{fin_fr}"), height = 1017, width = 1920, pointsize = 20)
    barplot(m, main = str_glue("{debut}{matrix_name[i]}{fin}{uni_gene[10,]}")) # Barblot de la presence du gene donne dans la matrice
    dev.off()
    png(str_glue("{deb_en}{matrix_name[i]}{fin_en}"), height = 1017, width = 1920, pointsize = 20)
    barplot(m, main = str_glue("{uni_gene[10,]}{start}{matrix_name[i]}{end}")) # Barblot de la presence du gene donne dans la matrice
    dev.off()
  }
}